...
import { discoverTv, fetchTvGenres } from '../../services/tmdb/tmdbApi';
import type { DiscoverTvSort, TvListItem } from '../../services/tmdb/types';
import { ErrorState } from '../../shared/components/ErrorState';
import { Screen } from '../../shared/components/Screen';

type Nav = NativeStackNavigationProp<HomeStackParamList, 'DiscoverTv'>;

const SORT_OPTIONS: { key: DiscoverTvSort; labelKey: string }[] = [
  { key: 'popularity.desc', labelKey: 'discover.sortPopularity' },
  { key: 'vote_average.desc', labelKey: 'discover.sortRating' },
  { key: 'first_air_date.desc', labelKey: 'discover.sortNew' },
];
...
export function DiscoverTvScreen() {
  ...
  const [sortBy, setSortBy] = useState<DiscoverTvSort>('popularity.desc');
  ...
  const discoverQuery = useInfiniteQuery({
    queryKey: ['tmdb', 'discover', 'tv', genreId, yearNum, sortBy],
    queryFn: ({ pageParam }) =>
      discoverTv({
        page: pageParam,
  ...
  if (discoverQuery.isError && shows.length === 0) {
  ...
  const filtersHeader = (
    <View style={styles.filters}>
      ...
      <TextInput
        ...
        accessibilityLabel={i18n.t('discover.firstAirYear')}
      />
